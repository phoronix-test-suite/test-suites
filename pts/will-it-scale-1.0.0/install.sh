...
case \$@ in
	...
	;;
	\"EVENT_FD1\")
		./event_fd1_processes -s \$DURATION -t \$NUM_CPU_CORES -n > \$LOG_FILE 2>&1
	;;
	\"FALLOCATE1\")
	...
	;;
	\"LSEEK2\")
		./lseek3_processes -s \$DURATION -t \$NUM_CPU_CORES -n > \$LOG_FILE 2>&1
	;;
	\"MALLOC1\")
	...
	;;
	\"PAGE_FAULT2\")
		./page_fault_processes -s \$DURATION -t \$NUM_CPU_CORES -n > \$LOG_FILE 2>&1
	;;
	\"PAGE_FAULT3\")
	...
	;;
	\"POLL2\")
		./poll_processes -s \$DURATION -t \$NUM_CPU_CORES -n > \$LOG_FILE 2>&1
	;;
	\"POSIX_SEMAPHORE1\")
	...
	;;
	\"PREAD2\")
		./pread_processes -s \$DURATION -t \$NUM_CPU_CORES -n > \$LOG_FILE 2>&1
	;;
	\"PREAD3\")
...
echo \$? > ~/test-exit-status" > will-it-scale-run

chmod +x will-it-scale-run
